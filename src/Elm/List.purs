...
{-| Reduce a list from the left, building up all of the intermediate results into a list.
    ...
    Note that the Elm version of `scanl` includes the initial value (as in the example),
    so we preserve that behaviour here.
-}
...
partition pred list =
    foldr step { trues: Nil, falses: Nil } list 
        where
            step x memo =
                if pred x then
                    memo { trues = x : memo.trues }
                else
                    memo { falses = x : memo.falses }
